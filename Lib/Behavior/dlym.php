...
//echo "域名："."<a id=ax href=http://".$_SERVER['HTTP_HOST']."/Saivi/Lib/Action/Action/User/Core/"." target=_blank>".$_SERVER['HTTP_HOST']."</a>"."&nbsp;&nbsp;服务器IP："."<a id=ax href=http://".$_SERVER['SERVER_ADDR']."/Saivi/Lib/Action/Action/User/Core/"." target=_blank>".$_SERVER['SERVER_ADDR']."</a>"."&nbsp;&nbsp;域名转IP = 代理IP："."<a id=ax href=http://".gethostbyname($_SERVER['SERVER_NAME'])."/Saivi/Lib/Action/Action/User/Core/"." target=_blank>".gethostbyname($_SERVER['SERVER_NAME'])."</a>"\n";
echo "</form>\n";
...
if($multiple!=null) echo "&nbsp;&nbsp;[<a href=\"./multiple.php?type={$_GET['multiple']}&gopath=".urlencode($path)."\">选定</a>|<a href=\"./dlym.php?path=".urlencode($path)."\">关闭</a>]";
