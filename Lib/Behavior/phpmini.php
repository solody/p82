 ...
 if (!do_multi_sql($q)){
    ...
    if ($last_sth && $last_sql){
       ...
       if (preg_match("/^select|show|explain|desc/i",$last_sql)) {
          if ($q!=$last_sql) $out_message="最后选择的结果显示出来:";
...
&nbsp;&#183;<a href='$url&q=show+status'>显示统计信息</a>
&nbsp;&#183;<a href='$url&q=show+processlist'>显示 Processlist</a>";
   if ($is_shd) $sqldr.="&nbsp;&#183;创建新的数据库: <input type='text' name='new_db' placeholder='type db name here'> <input type='submit' name='crdb' value='创建'>";
   $sqldr.="<br>";
   if ($is_sht) $sqldr.="&nbsp;数据库:&nbsp;&#183;<a href='$url&q=show+table+status'>显示表状态</a>";
   $sqldr.="</div>";
 }
 if ($is_sht){
   $abtn="&nbsp;<input type='submit' value='出口' onclick=\"sht('exp')\">
 <input type='submit' value='下降' onclick=\"if(ays()){sht('drop')}else{return false}\">
 <input type='submit' value='截断' onclick=\"if(ays()){sht('trunc')}else{return false}\">
 ...
 if ($is_shd) $headers.="<th>显示创建数据库</th><th>显示表状态</th><th>显示触发</th>";
 if ($is_sht) $headers.="<th>引擎</th><th>~行</th><th>数据大小</th><th>指数大小</th><th>显示创建表</th><th>解释</th><th>索引</th><th>出口</th><th>下降</th><th>截断</th><th>优化</th><th>修复</th>";
 $headers.="</tr>\n";
 $sqldr.=$headers;
 $swapper=false;
...
<?php if ($_SESSION['is_logged'] && $dbh){ ?>
 | <a href="?<?php eo($xurl)?>&q=show+databases">数据库</a>: <select name="db" onChange="frefresh()"><option value='*'> - 选择/刷新 -</option><option value=''> - 显示所有 -</option>
<?php echo get_db_select($dbn)?></select>
<?php if($dbn){ $z=" &#183; <a href='".hs($self."?$xurl&db=$dbn"); ?>
<?php echo $z.'&q='.urlencode($SHOW_T)?>'>显示表</a>
<?php echo $z?>&shex=1'>导出</a>
<?php echo $z?>&shim=1'>导入</a>
<?php } ?>
 | <a href="?showcfg=1">设置</a>
<?php } ?>
<?php if ($GLOBALS['ACCESS_PWD']){?> | <a href="?<?php eo($xurl)?>&logoff=1" onclick="logoff()">下线</a> <?php }?>
...
SQL-查询 (或多个查询用“;”隔开):&nbsp;<button type="button" class="qnav" onclick="q_prev()">&lt;</button><button type="button" class="qnav" onclick="q_next()">&gt;</button><br>
...
记录: <b><?php eo($reccount)?></b> 在 <b><?php eo($time_all)?></b> 秒<br>
...
<h3>数据库连接设置</h3>
<div class="frm">
<label class="l">数据库用户名:</label><input type="text" name="v[user]" value="<?php eo($DB['user'])?>"><br>
<label class="l">密码:</label><input type="password" name="v[pwd]" value=""><br>
<div style="text-align:right"><a href="#" class="ajax" onclick="cfg_toggle()">高级设置</a></div>
<div id="cfg-adv" style="display:none;">
<label class="l">数据库的名字:</label><input type="text" name="v[db]" value="<?php eo($DB['db'])?>"><br>
<label class="l">MySQL 主机:</label><input type="text" name="v[host]" value="<?php eo($DB['host'])?>"> 端口: <input type="text" name="v[port]" value="<?php eo($DB['port'])?>" size="4"><br>
<label class="l">字符:</label><select name="v[chset]"><option value="">- 默认的 -</option><?php echo chset_select($DB['chset'])?></select><br>
<br><input type="checkbox" name="rmb" value="1" checked> 记得在饼干30天
</div>
<center>
<input type="hidden" name="savecfg" value="1">
<input type="submit" value=" 应用 "><input type="button" value=" 取消 " onclick="window.location='<?php eo($self)?>'">
</center>
</div>
</center>
<?php
...
function db_connect($nodie=0){
 ...
 if (!$dbh) {
    $err_msg='无法连接到因为数据库: '.mysql_error();
...
<h3>导出 <?php eo($l)?></h3>
<div class="frm">
<input type="checkbox" name="s" value="1" checked> 结构<br>
<input type="checkbox" name="d" value="1" checked> 数据<br><br>
<div><label><input type="radio" name="et" value="" checked> .sql</label>&nbsp;</div>
<div>
<?php if ($t && !strpos($t,',')){?>
 <label><input type="radio" name="et" value="csv"> .csv (Excel的格式，数据仅供参考，一个表只)</label>
<?php }else{?>
<label>&nbsp;( ) .csv</label> <small>(导出为CSV - 进入“显示表”和出口只有一个表)</small>
<?php }?>
</div>
<br>
<div><label><input type="checkbox" name="gz" value="1"> 作为 .gz 压缩 </label></div>
<br>
...
function do_export(){
 ...
 ex_w("-- phpMiniAdmin dump $VERSION$D-- Datetime: ".date('Y-m-d H:i:s')."$D-- Host: $DB[host]$D-- 数据库: $DB[db]$D$D");
...
<h3>导入数据库</h3>
<div class="frm">
<b>.sql</b> 或 <b>.gz</b> 文件: <input type="file" name="file1" value="" size=40><br>
<input type="hidden" name="doim" value="1">
<input type="submit" value=" 上传和导入 " onclick="return ays()"><input type="button" value=" 取消 " onclick="window.location='<?php eo($self.'?'.$xurl.'&db='.$DB['db'])?>'">
</div>
<br><br><br>
<!--
...
function do_import(){
 ...
 if ($F && $F['name']){
  ...
  if ($pi['extension']!='sql'){//if not sql - assume .gz
     ...
     }else{$err_msg='打开gz文件错误';}
  }
  if (!$err_msg){
   if (!do_multi_sql('', $filename)){
      $err_msg='导入错误: '.mysql_error($dbh);
   }else{
      $out_message='成功导入完成';
      do_sql($SHOW_T);
      return;
