...
class yinmeili
{
    private $api = 'http://s1.api.yinmeili.com/index.php';
    private $username = '';
    private $secret = '';
    private $unique_id = '';
    private $uid = '';

    public function __construct($username = '', $secret = '', $unique_id = '', $uid =
        '')
    {
    ...
    public function updatePrintConfig($mid = 0, $print_model = '', $is_pay = 0, $limit_type =
        0, $print_limit = 0, $small_banner = '', $banner = '', $readme = '', $logo = '',
        $footer_url = '', $qrcode = '')
    {
        $mid = intval($mid);

        $is_pay = intval($is_pay);
        $limit_type = intval($limit_type);
    ...
    public function addVoice($appid = '', $appsecret = '', $tid = 0, $media_id = '')
    {
        $appid = trim($appid);
        ...
        {
            $output['status'] = 1;
            $output['message'] = '留声卡语音添加成功,输入验证码完成照片打印,输入退出离开照片打印!';
            return $output;
        }
        else
        {
    ...
    public function checkPrintVerifyCode($tid = 0, $verify_code = '')
    {
        ...
        if ($this->username == '' || $this->secret == '' || $tid == 0)
        {
            $output['message'] = '未找到打印任务!';
            return $output;
        }
    ...
    public function http($params = array(), $type = 'post', $uri = '')
    {
        if ($uri == '')
        {
            $uri = $this->api;
