...
//dump($_SESSION['token_openid_kvdhyj1396063025']='sfe110');
		if(($share_code && $is_see == 1) || ($share_code && empty($this->wecha_id) && empty($is_see))){
		...
		if(!empty($this->fans) && empty($share_code) && empty($puser) && empty($is_see)){	
			...
			if($puser){
				...
				$puser['portrait'] 		= $uinfo['portrait'] ? $uinfo['portrait'] : $this->siteUrl.'/tpl/static/Popularity/images/yk.jpg';
	...
	public function rank(){
		...
		foreach($rank as $key=>$val){
			...
			$rank[$key]['portrait']		= $user['portrait']?$user['portrait'] : $this->siteUrl.'/tpl/static/Popularity/images/yk.jpg';
		}
	...
	public function getShare($key=""){
		...
		foreach($list as $key=>$val){
			...
			if(empty($user)){
				...
				$list[$key]['portrait']		= $this->siteUrl.'/tpl/static/Popularity/images/yk.jpg';
			}else{
				$list[$key]['username']		= $user['wechaname'] ? $user['wechaname'] : '匿名';
				$list[$key]['portrait']		= $user['portrait'] ? $user['portrait'] : $this->siteUrl.'/tpl/static/Popularity/images/yk.jpg';
			}
