...
class ActivityBaseAction extends UserAction{
	...
	public function index($type){
		...
		$lottery=M('Lottery')->where(array('zjpic'=>$list['id']))->order('id DESC')->getField('id');	
			...
			if($_POST['enddate'] < $_POST['statdate']){
				...
				if($data->create()!=false){
					if($id=$data->add()){
						...
						M('Lottery')->add($data2);
						$this->handleKeyword($id,'Activity',$this->_POST('keyword'));
			...
			if($_POST['enddate'] < $_POST['statdate']){
				...
				if($check==false)$this->error('非法操作');
					if($data->where($where)->save($_POST)){
						...
						$this->handleKeyword($lid,'Activity',$this->_POST('keyword'));
	...
	public function exportSN(){
		...
		if ($sns){
			...
			$user =  M('Userinfo')->where(array('token'=>$this->token,'wecha_id'=>$val['wecha_id']))->field('wechaname,tel')->find();
				$sns[$key]['tel'] = $user['tel'];
				$sns[$key]['name'] = $user['wechaname'];
			}
			foreach ($sns as $sn){
				$j=0;
